#include <Servo.h>

// Definitii pini
int WhiteLedPin = 5;           // Pin pentru LED-ul alb
int RedLedPin = 6;             // Pin pentru LED-ul rosu
int GreenLedPin = 7;           // Pin pentru LED-ul verde
int CountButtonPin = 15;       // Buton pentru numarare apasari
int StartButtonPin = 14;       // Buton pentru pornirea procesului
int UpWhiteButtonPin = 16;     // Buton pentru miscarea motorului in sus
int DownRedButtonPin = 17;     // Buton pentru miscarea motorului in jos
int ServoPin = 11;             // Pin de control pentru servo

// Variabile
Servo myServo;                  // Obiect servo
int cnt = 0;                     // Contor pentru apasarile butonului
int min = 0;                     // Durata procesului calculata pe baza apasarilor
int starePinCountButton = 0;     // Stare pentru debouncing la butonul de numarare
int starePinStartButton = 0;     // Stare pentru debouncing la butonul de start
int starePinUpWhiteButton = 0;   // Stare pentru debouncing la butonul alb
int starePinDownRedButton = 0;   // Stare pentru debouncing la butonul rosu
int white = 1;                   // Starea LED-ului alb
int start = 0;                   // Indicator pentru proces in desfasurare

void setup() {
  // Configurare pini
  pinMode(WhiteLedPin, OUTPUT);
  pinMode(RedLedPin, OUTPUT);
  pinMode(GreenLedPin, OUTPUT);

  pinMode(CountButtonPin, INPUT);
  pinMode(StartButtonPin, INPUT);
  pinMode(UpWhiteButtonPin, INPUT);
  pinMode(DownRedButtonPin, INPUT);

  // Initializare servo
  myServo.attach(ServoPin);             // Ataseaza servo-ul la pin
  myServo.writeMicroseconds(1500);      // Pozitie neutra (stop)

  // Stare initiala a LED-urilor
  digitalWrite(WhiteLedPin, HIGH);      // LED-ul alb ON initial
  digitalWrite(GreenLedPin, LOW);
  digitalWrite(RedLedPin, LOW);
}

void loop() {
  whiteLed();                     // Controleaza starea LED-ului alb
  if (start == 0) counting();     // Numara apasarile butonului cand procesul nu este pornit

  // Control pentru UpWhiteButton - Miscare in sens orar cat timp butonul este apasat
  if (digitalRead(UpWhiteButtonPin) == HIGH) {
     starePinUpWhiteButton = 1;              // Stare buton activata
     myServo.writeMicroseconds(1200);       // Misca servo-ul in sens orar
     delay(20);
  }  else {
      starePinUpWhiteButton = 0;
      myServo.writeMicroseconds(1500);      // Opreste servo-ul cand butonul este eliberat
  }

  // Control pentru DownRedButton - Miscare in sens antiorar cat timp butonul este apasat
  if (digitalRead(DownRedButtonPin) == HIGH) {
     starePinDownRedButton = 1;            // Stare buton activata
     myServo.writeMicroseconds(1800);     // Misca servo-ul in sens antiorar
     delay(20);
  }  else {
      starePinDownRedButton = 0;
      myServo.writeMicroseconds(1500);    // Opreste servo-ul cand butonul este eliberat
  }

  // Pornirea procesului prin apasarea butonului Start
  if (digitalRead(StartButtonPin) == HIGH) {
    if (starePinStartButton == 0) {
      starePinStartButton = 1;
      start = 1;
      digitalWrite(RedLedPin, HIGH);       // Aprinde LED-ul rosu pentru a indica inceputul procesului

      min = cnt;                    // Convertirea contorului in milisecunde
      cnt = 0;                             // Reseteaza contorul

      moveMotor(min);                      // Executa miscarea motorului pentru timpul calculat

      start = 0;
      white = 1;
      digitalWrite(RedLedPin, LOW);        // Stinge LED-ul rosu dupa finalizarea procesului
      ready_anim();                        // Animatie de finalizare
    }
  } else {
    starePinStartButton = 0;
  }
}

// Functie pentru a misca motorul pentru un timp specificat T
void moveMotor(int T) {
  for(int i = 0; i < 3; i++) {             // Oscilatie initiala a motorului
    myServo.writeMicroseconds(1800);      // Miscare inainte
    delay(1000);                          // Asteapta
    myServo.writeMicroseconds(1200);      // Miscare inapoi
    delay(1000);                          // Asteapta
  }
  myServo.writeMicroseconds(1800);       // Miscare inainte
  delay(1000);
  myServo.writeMicroseconds(1500);       // Opreste motorul
  for(int i = 0; i < T * 60; i++) {
    Serial.print(i);    
    delay(1000);                     // Asteapta pentru timpul T
  }
  myServo.writeMicroseconds(1200);       // Miscare inapoi
  delay(1500);                           // Asteapta
  myServo.writeMicroseconds(1500);       // Opreste motorul
}

// Functie pentru numararea apasarilor butonului
void counting() {
  if (digitalRead(CountButtonPin) == HIGH) {
    if (starePinCountButton == 0) {
      starePinCountButton = 1;
      cnt++;                             // Incrementeaza contorul
      white = 0;                         // Stinge LED-ul alb
      digitalWrite(GreenLedPin, HIGH);   // Aprinde LED-ul verde
    }
  } else {
    starePinCountButton = 0;
    digitalWrite(GreenLedPin, LOW);      // Stinge LED-ul verde
  }
}

// Functie pentru controlul LED-ului alb
void whiteLed() {
  if (white == 1) {
    digitalWrite(WhiteLedPin, HIGH);    // LED-ul alb aprins
  } else {
    digitalWrite(WhiteLedPin, LOW);     // LED-ul alb stins
  }
}

// Animatie de finalizare
void ready_anim() {
  digitalWrite(RedLedPin, LOW);

  for (int i = 0; i < 5; i++) {
    digitalWrite(WhiteLedPin, HIGH);
    delay(250);                         // LED-ul alb clipind
    digitalWrite(WhiteLedPin, LOW);
    delay(250);
  }
}
